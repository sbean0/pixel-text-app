...
function setupDragAndDrop() {
    ...
    containers.forEach(container => {
        ...
        container.addEventListener('drop', e => {
            ...
            if (container.id === 'activeFonts') generateText(); // Regenerate on drop
        });
    });
...
function generateText() {
    ...
    if (activeFonts.length === 0) {
        alert("Please drag at least one font into the Active Fonts area!");
        return;
    }
    ...
    activeFonts.forEach((font, index) => {
        ...
        const y = qrHeight + (index * sectionHeight) + (sectionHeight / 2);

        ctx.fillText(userText, x, y);

        if (index < activeFonts.length - 1) {
            ctx.strokeStyle = '#000';
            ctx.lineWidth = 2;
            ctx.beginPath();
...
function addCustomFont() {
    ...
    if (currentFonts >= 5) {
        alert("Maximum of 5 fonts allowed in Active Fonts! Remove one first.");
        return;
    }
